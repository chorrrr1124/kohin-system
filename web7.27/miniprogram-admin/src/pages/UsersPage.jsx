...
const CustomersPage = () => {
  ...
  const fetchAllCustomers = async () => {
    try {
      ...
      filterAndPaginateCustomers(result.data);
    } catch (error) {
  ...
  // 本地筛选和分页处理
  const filterAndPaginateCustomers = (data = allCustomers) => {
    ...
    if (natureFilter) {
      filteredCustomers = filteredCustomers.filter(customer => 
        getCustomerNatureDisplay(customer) === natureFilter
      );
    }
  ...
  }, [currentPage, searchTerm, natureFilter, showStats, allCustomers]);
    ...
    try {
      ...
      result.data.forEach(customer => {
        csvData.push([
          customer._id,
          customer.name || '',
          getCustomerNatureDisplay(customer),
  ...
  const startEditCustomer = (customer) => {
    ...
    setCustomerForm({
      name: customer.name || '',
      nature: getCustomerNatureDisplay(customer),
              ...
              <table className="table table-zebra w-full">
                ...
                <tbody>
                  {customers.map((customer) => (
                    <tr key={customer._id}>
                      {false && <td className="font-mono text-sm">{customer._id?.slice(-8)}</td>}
                      <td className="font-semibold">{customer.name}</td>
                      <td>
                        <span className={`badge ${
                          getCustomerNatureDisplay(customer) === '金额预存客户' ? 'badge-success' :
                          getCustomerNatureDisplay(customer) === '产品预存客户' ? 'badge-info' :
                          'badge-warning'
                        }`}>
                          {getCustomerNatureDisplay(customer)}
                        </span>
          ...
          <div className="modal-box max-w-2xl">
            ...
            <div className="mb-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                {selectedCustomer.nature && <div><span className="font-semibold">客户类型：</span>{getCustomerNatureDisplay(selectedCustomer)}</div>}
