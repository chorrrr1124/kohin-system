  ...
  const loginUser = async () => {
    try {
      ...
      if (hasUser) {
        ...
        addLog(`❌ 登录状态异常: isLoggedIn=${isLoggedIn}, isOffline=${isOffline}, hasUser=${hasUser}`, 'error');
    ...
    try {
      ...
      for (let i = 0; i < selectedFiles.length; i++) {
        ...
        try {
          ...
          const pathPrefix = category ? category.path : 'images/general/';
          
          // 生成完整的文件路径
          const fileKey = generateFileKey(file.name, pathPrefix);
          
          // 上传到COS，添加进度回调
          const result = await uploadToCos(file, fileKey, (progressData) => {
