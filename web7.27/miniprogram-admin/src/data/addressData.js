...
export const districtsData = {
  ...
  '500100': [
    ...
    { code: '500235', name: '奉节县' },
    { code: '500236', name: '巫山县' },
    { code: '500237', name: '巫溪县' },
    { code: '500238', name: '石柱土家族自治县' },
    { code: '500240', name: '秀山土家族苗族自治县' },
    { code: '500241', name: '酉阳土家族苗族自治县' },
    { code: '500242', name: '彭水苗族土家族自治县' }
  ],
  // 广东省 - 广州市
  '440100': [
...
export const AddressUtils = {
  // 根据地址文本识别省市区
  recognizeAddress: (addressText) => {
    if (!addressText) return { province: '', city: '', district: '', detail: '' };
    
    const result = { province: '', city: '', district: '', detail: addressText };
    
    // 遍历所有省份
    for (const province of addressData.provinces) {
      if (addressText.includes(province.name)) {
        result.province = province.name;
        
        // 查找对应的城市
        const cities = addressData.cities[province.code] || [];
        for (const city of cities) {
          if (addressText.includes(city.name)) {
            result.city = city.name;
            
            // 查找对应的区县
            const districts = districtsData[city.code] || [];
            for (const district of districts) {
              if (addressText.includes(district.name)) {
    ...
    if (result.province) detail = detail.replace(result.province, '');
    if (result.city) detail = detail.replace(result.city, '');
    if (result.district) detail = detail.replace(result.district, '');
  ...
  // 根据地址数组获取地址文本
  getAddressText: (addressArray) => {
    if (!addressArray || addressArray.length === 0) return '';
    return addressArray.join('');
  },
  
  // 根据地址数组获取地址对象
  getAddressObject: (addressArray) => {
