  ...
  saveAddress: function () {
    ...
    handleDefaultAddress()
      ...
      .then((res) => {
        ...
        console.log('地址保存成功，准备传回购物车:', savedAddress);
        ...
        try {
          ...
          if (pages.length > 1) {
            ...
            console.log('上一页面路径:', prevPage.route);
            
            // 如果上一页是购物车页面，直接设置选中地址
            if (prevPage.route && prevPage.route.includes('cart')) {
              console.log('检测到上一页是购物车，直接设置选中地址');
            ...
            // 如果上一页是地址列表页，刷新地址列表
            if (prevPage.route && prevPage.route.includes('addressList')) {
              if (typeof prevPage.loadAddressList === 'function') {
              ...
              if (eventChannel) {
                eventChannel.emit('addressSelected', savedAddress);
              }
            }
          }
        } catch (err) {
          console.error('传递地址到上一页面失败:', err);
        }
    ...
    db.collection('customers').doc(address.customerId).get().then(res => {
      ...
      if (customer && customer.contacts) {
        ...
        if (typeof customer.contacts === 'string') {
          ...
          contacts = [];
          console.log('contacts类型未知，初始化为空数组');
        }
      }
      ...
      for (let i = 0; i < contacts.length; i++) {
        if (contacts[i].phone === address.phone) {
          ...
          console.log('更新现有联系人:', updatedContact);
          break;
        }
      }
  ...
  tryBackupAddressSync: function(address) {
    ...
    db.collection('customers').doc(address.customerId).get()
      .then(res => {
        ...
        let contacts = [];
        if (customer && customer.contacts) {
          console.log('备用方法-原始contacts数据:', customer.contacts, '类型:', typeof customer.contacts);
          
          if (typeof customer.contacts === 'string') {
            try {
              contacts = JSON.parse(customer.contacts);
              console.log('备用方法-解析JSON字符串后的contacts:', contacts);
            } catch (e) {
              console.error('备用方法-解析contacts JSON失败:', e);
              contacts = [];
            }
          } else if (Array.isArray(customer.contacts)) {
            contacts = customer.contacts;
            console.log('备用方法-contacts已经是数组:', contacts);
          } else {
            contacts = [];
            console.log('备用方法-contacts类型未知，初始化为空数组');
          }
        }
        let updated = false;
